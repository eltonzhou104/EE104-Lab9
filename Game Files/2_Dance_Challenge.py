...
def draw():
    global game_over, p2_score, p1_score, say_dance
    ...
    if not game_over:
        ...
        right.draw()
        left.draw()
        screen.draw.text("Player 1 Score: " + str(p1_score), color="black", topleft=(10, 10))
        screen.draw.text("Player 2 Score: " + str(p2_score), color="black", topleft=(10, 30))
        screen.draw.text("Round " + str(rounds), color="black", topleft=(10, 90))
        if say_dance: screen.draw.text("Dance!", color="black", topleft=(CENTER_X - 65, 150), fontsize=60)
        if show_countdown:
            if rounds % 2 == 0: player = 2
            else: player = 1
            screen.draw.text("Player " + str(player) + " Playing", color="black", topleft=(CENTER_X - 99, 110), fontsize=40)
...
def generate_moves():
    ...
    rounds+=1
    if (rounds % 3 == 0): dance_length = dance_length + 1
    count = 4
    move_list = []
    say_dance = False
...
def next_move():
    ...
    if current_move < dance_length - 1: current_move = current_move + 1
    else: moves_complete = True
    return

def on_key_up(key):
    global p2_score, p1_score, game_over, move_list, current_move, rounds
    if rounds % 2 == 0:
        if key == keys.UP:
            update_dancer(0)
